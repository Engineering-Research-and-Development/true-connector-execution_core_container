...
public class PlatoonUsageControlServiceImplTest {
	...
	public void testEnforceUsageControl_Succesfull() throws IOException {
		...
		StringBuffer ucUrl = new StringBuffer().append(platoonURL)
				.append("enforce/usage/agreement")
				.append("?contractAgreementUri=")
				.append(ucURI)
				.append("&consuming=true");
		
		when(communicationService.sendDataAsJson(ucUrl.toString(), ucObjet, "application/json;charset=UTF-8")).thenReturn("Usage allowed");
		
		String response = platoonUsageControlServiceImpl.enforceUsageControl(ucURI, ucObjet);
	...
	public void testEnforceUsageControl_Failed() throws IOException {
		...
		when(communicationService.sendDataAsJson(ucUrl.toString(), ucObjet, "application/json;charset=UTF-8")).thenReturn("Usage prohibited: Not valid contract agreement");
	...
	public void testCreateUsageControlObject () {
		...
		String ucObject = platoonUsageControlServiceImpl.createUsageControlObject(UtilMessageService.getArtifactRequestMessage(), 
